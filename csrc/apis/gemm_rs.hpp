...
#include "../jit_kernels/impls/sm100_fp8_gemm_rs.hpp"
#endif
#if DG_TENSORMAP_COMPATIBLE
#include "../jit_kernels/impls/sm100_bf16_gemm_rs.hpp"
#include "../jit_kernels/impls/sm100_bf16_gemm_rs_fused.hpp"
...
static void fp8_gemm_rs_nt(const torch::Tensor& y,
                           const std::tuple<torch::Tensor, torch::Tensor>& a_tuple,
    ...
    const auto [m, k] = check_ab_fp8_fp4(a, major_a, arch_major);
    const auto [n, k_] = check_ab_fp8_fp4(b, major_b, arch_major);
    ...
    auto recipe_opt = std::optional<std::tuple<int, int, int>>(recipe);
...
static void bf16_gemm_rs_fused(const torch::Tensor& y,
                              const torch::Tensor& a,
                              const torch::Tensor& b,
                              const torch::Tensor& sym_buffer,
                              const std::vector<int64_t>& sym_buffer_ptrs,
    ...
    sm100_bf16_gemm_rs_fused(y, a, b, sym_buffer, sym_buffer_ptrs, rank_idx,
                             num_max_tokens_per_rank, num_tokens_per_rank, n, k, compiled_dims,
                             comm_dtype, reduce_in_fp32);
#else
    DG_HOST_UNREACHABLE("BF16 GEMM+RS fused requires TensorMap support");
#endif
}

// ════════════════════════════════════════════════════════════════
//  V2: Single-kernel Pull-based GEMM + RS
// ════════════════════════════════════════════════════════════════
static void bf16_gemm_rs_v2_nt(const torch::Tensor& y,
    ...
    sm100_bf16_gemm_rs_v2_nt(y, a, b, sym_buffer, sym_buffer_ptrs, rank_idx,
                             ...
                             comm_dtype);
#else
    DG_HOST_UNREACHABLE("BF16 GEMM+RS V2 requires TensorMap support");
#endif
}
...
static void register_apis(pybind11::module_& m) {
    ...
    m.def("fp8_gemm_rs_nt", &fp8_gemm_rs_nt,
    ...
    m.def("bf16_gemm_rs_nt", &bf16_gemm_rs_nt,
          ...
          pybind11::arg("comm_dtype") = "bf16",
          pybind11::arg("reduce_in_fp32") = true);
    m.def("bf16_gemm_rs_fused", &bf16_gemm_rs_fused,
          pybind11::arg("y"), pybind11::arg("a"), pybind11::arg("b"), pybind11::arg("sym_buffer"),
          pybind11::arg("sym_buffer_ptrs"), pybind11::arg("rank_idx"),
          pybind11::arg("num_max_tokens_per_rank"), pybind11::arg("num_tokens_per_rank"),
          pybind11::arg("compiled_dims") = "nk",
