...
sm100_fp8_gemm_rs_nt_impl(void* y,
    ...
    DG_STATIC_ASSERT(BLOCK_M == 128 and BLOCK_N == 128 and BLOCK_K == 128, "The first GEMM+RS version expects 128x128x128 tiles");
    ...
    constexpr uint32_t kAfterReadyCleanBarrierTag = 31;
        ...
        while (get_next_block(block_idx, m_block_idx, n_block_idx, iter_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                if (sfa_stage_in_group_idx == 0 and cute::elect_one_sync()) {
                    auto smem_ptr = smem_sfa[stage_idx];
                ...
                if (sfb_stage_in_group_idx == 0 and cute::elect_one_sync()) {
                    auto smem_ptr = smem_sfb[stage_idx];
                    replace_smem_desc_addr(sf_desc, smem_ptr);
        ...
        while (get_next_block(block_idx, m_block_idx, n_block_idx, iter_idx)) {
            ...
            for (uint32_t w = 0; w < kNumMWaves; ++ w) {
                ...
                for (uint32_t s = 0; s < kNumStores; ++ s, advance_store_pipeline()) {
                    ...
                    const uint32_t partial_m = dst_rank * shape_m_per_rank + local_m + w * WAVE_BLOCK_M;
                    ...
                    // All2All-style scatter: push the partial tile directly from epilogue smem to
                    // the destination rank's local-reduce slot. No local partial materialization.
