"""Throughput sweep: serial vs fused_var vs fused_var_v2 across sequence lengths.

Sweeps sequence lengths [2K, 4K, 8K, 16K, 32K] with full 14B model,
measuring FWD/BWD/wall-clock/peak-memory for all three strategies.
...
def run(rank, world_size, port, args, checkpoint_dir):
    ...
    if rank == 0:
        print(f'\n{"="*120}')
        print(f'Ulysses Variant v2 Throughput Sweep — Wan2.1 T2V-14B ({args.layers} layers)')
        print(f'Hardware: B300 x{world_size}, SP={sp}, sync_mode={args.sync_mode}')
        print(f'Strategies: {strategies}')
        print(f'{"="*120}')
        print(f'{"Strategy":<16} {"Seq":>6} {"FWD(ms)":>10} {"BWD(ms)":>10} '
    ...
    for seq_len in seq_lengths:
        ...
        for strategy in strategies:
            ...
            if rank == 0:
                print(f'{strategy:<16} {seq_len//1024:>4}K {fwd:>9.2f} {bwd:>9.2f} '
                      f'{total:>9.2f} {wall:>9.2f} {tps:>9.0f} {peak_mb:>9.0f}', flush=True)
