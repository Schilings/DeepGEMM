"""Plot 1000-step training stability: loss curve + throughput for v2."""
import os, json
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
v_loss = data['fused_var_v2']['losses']
steps = list(range(len(s_loss)))

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

ax1.plot(steps, s_loss, '-', color=C_SERIAL, linewidth=2, label='serial (baseline)', alpha=0.9)
ax1.plot(steps, v_loss, '-', color=C_V2, linewidth=2, label='fused_var_v2 (native AG+overlap)', alpha=0.9)
ax1.set_xlabel('Training Step', fontsize=12)
ax1.set_ylabel('Loss (MSE)', fontsize=12)
ax1.set_title('1000-Step Training Loss: serial vs fused_var_v2\n'
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, 'fig_v2_train_1000.png'), dpi=150)
plt.close()
print('Saved fig_v2_train_1000.png')
...
print(f'fused_var_v2: final_loss={v_loss[-1]:.6f}  avg_tps={data["fused_var_v2"]["avg_tps"]:.0f}  '
      f'peak={data["fused_var_v2"]["peak_mb"]/1024:.1f}GB')
print(f'Max loss diff: {max(diffs):.2f}%')
print(f'Final loss diff: {diffs[-1]:.2f}%')
print(f'Throughput ratio: {data["fused_var_v2"]["avg_tps"]/data["serial"]["avg_tps"]:.3f}x')
print(f'Memory savings: {(data["serial"]["peak_mb"]-data["fused_var_v2"]["peak_mb"])/data["serial"]["peak_mb"]*100:.1f}%')
