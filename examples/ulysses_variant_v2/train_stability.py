"""1000-step training stability test for variant v2.

Compares serial baseline vs fused_var_v2 on:
- Loss curve (1000 steps, lr=1e-4, AdamW, official 14B weights)
- Throughput (tok/s)
- Peak memory
...
def _wrap_ddp(module, group, rank, bucket_cap_mb):
    ignored = [name for name, p in module.named_parameters()
               if getattr(p, '_sp_sharded', False) or getattr(p, '_deferred_grad', False)]
    if ignored:
...
class LatentDataLoader:
    def __init__(self, dim, seq_len, local_seq, device, seed=42):
        self.dim = dim
    ...
    def get_batch(self, step):
        ...
        freq_dim = 256
        g = torch.Generator(device=self.device).manual_seed(42 + 10000)
        t_proj = torch.randn(self.dim, freq_dim, device=self.device, dtype=torch.float32, generator=g) * 0.02
        t_emb = sinusoidal_embedding_1d(freq_dim, t).float()
        proj = torch.nn.functional.linear(t_emb, t_proj)
        e = proj.unsqueeze(1).expand(1, 6, self.dim)
        return x, e, context
...
def run(rank, world_size, port, args, checkpoint_dir):
    ...
    loader = LatentDataLoader(config.dim, seq_len, local_seq, dev, world_size)

    strategies = ['serial', 'fused_var_v2']
        ...
        for step in range(args.warmup):
            ...
            for p in model.parameters():
                if getattr(p, '_sp_sharded', False) and p.grad is not None:
                    p.grad.div_(sp)
                elif getattr(p, '_deferred_grad', False) and p.grad is not None:
    ...
    if rank == 0:
        ...
        v_loss = results['fused_var_v2']['losses']
        ...
        print(f'  fused_var_v2: final_loss={v_loss[-1]:.6f}  avg_tps={results["fused_var_v2"]["avg_tps"]:.0f}  '
              f'peak={results["fused_var_v2"]["peak_mb"]/1024:.1f}GB')
        print(f'  Max loss diff: {max_diff:.4%}')
        print(f'  Final loss diff: {final_diff:.4%}')
        print(f'  Throughput ratio: {results["fused_var_v2"]["avg_tps"]/results["serial"]["avg_tps"]:.3f}x')
        print(f'  Memory savings: {(results["serial"]["peak_mb"]-results["fused_var_v2"]["peak_mb"])/results["serial"]["peak_mb"]*100:.1f}%')
