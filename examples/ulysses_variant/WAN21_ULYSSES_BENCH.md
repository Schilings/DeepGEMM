...
- **lr=1e-4（微调场景）loss curve 完全一致**（0.0002%）：50 步训练，每步不同输入，loss 差异 < 0.001%
- **lr=1e-3（从头训练）loss curve 发散**：step 12 var loss 暴涨到 1.42（serial 0.96），step 20 飙到 1.87（serial 0.99）。Grad_W 4.44% 误差在高 lr 下被 AdamW 自适应学习率放大，导致训练不稳定
- **结论**：fused_var 在 **微调场景（lr ≤ 1e-4）下训练收敛性完全一致**，可安全使用。但在 **从头训练（lr ≥ 1e-3）下可能不稳定**，建议降低 lr 或使用 grad clip

### Loss curve 数据（lr=1e-4, 50 步, 不同输入）

| Step | serial loss | var loss | diff |
|-----:|---:|---:|---:|
| 0 | 0.99986 | 0.99986 | 0.0001% |
| 10 | 0.99858 | 0.99858 | 0.0001% |
| 20 | 0.99362 | 0.99362 | 0.0001% |
| 30 | 0.98890 | 0.98890 | 0.0000% |
| 45 | 0.98178 | 0.98178 | 0.0000% |
