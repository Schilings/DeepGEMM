...
small output head so the measurement isolates the transformer region affected by
this ablation.
"""
...
def run(rank, world_size, port, args, checkpoint_dir):
    ...
    import deep_gemm  # noqa: F401 - initializes DeepGEMM runtime bindings
    ...
    for strategy in args.strategies.split(","):
        ...
        if rank == 0:
            ...
            print(
                f"{'':<10} SP-synced={replicated / 1e9:.3f}B params, "
    ...
    if rank == 0 and "serial" in results and "fused_var" in results:
        serial_tps = results["serial"][1]
        variant_tps = results["fused_var"][1]
        delta = (variant_tps / serial_tps - 1.0) * 100.0
        print(
            f"\nThroughput verdict: fused_var / serial = {variant_tps / serial_tps:.4f}x "
            f"({delta:+.2f}%)",
            flush=True,
        )
...
def parse_args():
    ...
    parser.add_argument("--sync-mode", choices=("manual", "ddp"), default="manual")
