"""Plot loss curve comparison: serial vs fused_var."""
import os, json
import matplotlib
matplotlib.use('Agg')
...
ax1.plot(steps, var, 's-', color='#E85D3A', linewidth=2.5, markersize=7, label='fused_var (Wo sharded)')
ax1.set_xlabel('Training Step', fontsize=12)
ax1.set_ylabel('Loss (MSE)', fontsize=12)
ax1.set_title('Loss Curve: serial vs fused_var\n(4 layers, 8K seq, SP=8, lr=1e-4)', fontsize=13, fontweight='bold')
...
diffs = [abs(s - v) / (abs(s) + 1e-8) * 100 for s, v in zip(serial, var)]
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, 'fig_var6_loss_curve.png'), dpi=150)
plt.close()
print('Saved fig_var6_loss_curve.png')
