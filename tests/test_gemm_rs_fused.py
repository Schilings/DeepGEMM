"""
Validation test for BF16 GEMM + Reduce-Scatter 全融合 kernel (方案A).
单 kernel 内完成 compute + push + reduce，无需第二个 reduce kernel。
Usage: python test_gemm_rs_fused.py [num_gpus]  (default: 2)
"""
...
def run_test(local_rank: int, num_local_ranks: int):
    ...
    if rank_idx == 0:
        print(f"\n{'='*60}")
        print(f"BF16 GEMM-RS FUSED Test (方案A): {num_ranks} GPUs")
    ...
    deep_gemm.bf16_gemm_rs_fused(y, a, b, sym_buffer, tokens_per_rank, compiled_dims='nk')
    torch.cuda.synchronize(local_rank)
    dist.barrier()
    ...
    deep_gemm.bf16_gemm_rs_fused(y2, a, b, sym_buffer, tokens_per_rank, compiled_dims='nk')
    torch.cuda.synchronize(local_rank)
    dist.barrier()
    ...
    if rank_idx == 0:
        ...
        if max_diff < 1.0:
            print(f"  ✅ PASS — BF16 GEMM-RS FUSED matches reference!")
        elif max_diff < 5.0:
    ...
    # ── Also compare with the 2-kernel version (bf16_gemm_rs_nt) ──
    if rank_idx == 0:
        print("\n>>> Phase 4: Comparing with 2-kernel version (bf16_gemm_rs_nt)...")
    y3 = torch.zeros_like(y)
    deep_gemm.bf16_gemm_rs_nt(y3, a, b, sym_buffer, tokens_per_rank, compiled_dims='nk')
    torch.cuda.synchronize(local_rank)
    dist.barrier()

    diff_vs_2k = (y.float() - y3.float()).abs()
    max_diff_vs_2k = diff_vs_2k.max().item()
    if rank_idx == 0:
        print(f"  Fused vs 2-kernel max_diff: {max_diff_vs_2k:.6f}")
        if max_diff_vs_2k < 0.01:
            print(f"  ✅ PASS — Fused matches 2-kernel version!")
        else:
            print(f"  ❌ Difference detected (may be floating point ordering)")
