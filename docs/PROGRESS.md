...
| **A2A-GEMM** | `bf16_a2a_gemm_nt` | bench geo **1.176x**（8卡10it）| ⚠️ **3/6 FAIL** + **语义错位**：实现的是 token(M)-A2A，非 Ulysses post-attn → 待按 flux 重写（设计已就绪）| `main` | `A2A_TRANSPOSE_GEMM_DESIGN.md`（重写设计）/ `A2A_GEMM_ITERATION.md`（当前状态）/ `A2A_GEMM_DESIGN.md`（旧）|
