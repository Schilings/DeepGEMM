...
- **`FusedPreQKVFunction`（已完成）**：forward 用 `bf16_fused_qkv_norm_a2a_nt` + seq permute，backward 用 analytical norm-inverse 公式（文档行 97）避免 PyTorch autograd 开销。正确性：`grad_X rel=0.038, grad_Wq rel=0.038, grad_Nq rel=0.045`。性能：fused / serial = **1.084x (+8.38%)**，FWD 快 25ms。
