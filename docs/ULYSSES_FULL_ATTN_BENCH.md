...
| 链路 | PRE | ATTN | POST |
|---|---|---|---|
| **fused（ours）** | `bf16_gemm_a2a_transpose_nt` | FlashAttention-4 | `bf16_a2a_transpose_gemm_nt_fused`（重叠） |
| **torch-native** | `torch.matmul` + `all_to_all` | FlashAttention-4 | `all_to_all` + `torch.matmul` |
